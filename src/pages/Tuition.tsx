...
const SemesterCard: React.FC<{ 
  ...
  const reqDeletePayment = (pid: string) => {
    setDeleteModal({
      isOpen: true,
      message: "Are you sure you want to delete this payment record?",
      onConfirm: () => handleUpdateField('payments', payments.filter(p => p.id !== pid))
    });
  };

  const reqDeleteSemester = (e: React.MouseEvent) => {
    e.stopPropagation();
    setDeleteModal({
      isOpen: true,
      message: "Are you sure you want to delete this entire semester record?",
      onConfirm: () => deleteTuitionSemester(sem.id)
    });
  };
    ...
    <>
      ...
      <Card className="rounded-3xl border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden border-t-4 border-t-blue-500 transition-all duration-300">
        ...
        >
          ...
          <Button type="button" variant="ghost" size="icon" onClick={reqDeleteSemester}>
                    ...
                    {editingPayId === p.id ? (
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-1">
                          ...
                          <Button type="button" variant="ghost" size="icon" className="h-6 w-6" onClick={() => reqDeletePayment(p.id)}>
