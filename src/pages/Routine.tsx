...
const RoutineSemesterCard: React.FC<{ 
  ...
  const reqDeleteSemester = (e: React.MouseEvent) => {
    ...
    setDeleteModal({
      isOpen: true,
      message: "Are you sure you want to delete this entire semester routine?",
      onConfirm: () => onDeleteSemester(sem.id)
    });
  };

  const reqDeleteClass = (id: string) => {
    setDeleteModal({
      isOpen: true,
      message: "Are you sure you want to delete this class?",
      onConfirm: () => onDeleteClass(id)
    });
  };
                          ...
                          <div className="grid gap-3">
                            {dayClasses.map(cls => (
                              <div key={cls.id} className="group relative flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/40 rounded-2xl border border-slate-100 dark:border-slate-800 hover:border-emerald-200 dark:hover:border-emerald-900/50 transition-all">
                                ...
                                <Button type="button" variant="ghost" size="icon" className="text-slate-300 hover:text-red-500 transition-colors" onClick={() => reqDeleteClass(cls.id)}>
